...
begin
  ...
  try
    ...
    begin
      ...
      FHandle := CreateFile(PWideChar(FileName), GENERIC_READ or GENERIC_WRITE,
...
begin
  ...
  begin
    ...
    DeleteFile(FFileName + '.' + IntToStr(FMaxFileCount) + '.log');
    // Map (maxBackupIndex - 1), ..., 2, 1 to maxBackupIndex, ..., 3, 2
    for i := FMaxFileCount - 1 downto 1 do
    begin
      FileName := FFileName + '.' + IntToStr(i) + '.log';
      if FileExists(FileName) then
        RenameFile(FileName, FFileName + '.' + IntToStr(i+1) + '.log');
    end;
    // close file
    CloseFile;
    // Rename fileName to fileName.1
    RenameFile(FFileName, FFileName + '.1.log');
    // open new file
    OpenFile;
  end;
end;
