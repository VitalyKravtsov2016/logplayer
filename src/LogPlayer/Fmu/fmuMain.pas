...
uses
  ...
  PngImageList,
  // This
  untCommand, untLogPlayer, DrvFRLib_TLB, DriverCommands, LogParser2,
...
type
  TfmMain = class(TForm)
    ...
    procedure lvCommandsAdvancedCustomDrawItem(Sender: TCustomListView;
      Item: TListItem; State: TCustomDrawState; Stage: TCustomDrawStage;
      var DefaultDraw: Boolean);
    procedure lvCommandsAdvancedCustomDrawSubItem(Sender: TCustomListView;
...
begin
  ...
  Item.SubItems.Add(GetCommandShortValue(ACommand));
  Item.Data := ACommand;
...
begin
  ...
  repeat
    for i := current+1 to FCommands.Count - 1 do
    begin
    ...
    if Application.MessageBox('Начать с начала?', 'Поиск завершен', MB_OKCANCEL)
      = IDOK then
    begin
      current := 0;
      Continue;
  ...
  begin
    ...
    repeat
      if FCommands[current].HasData(edtSearch.Text) or
        (Pos(LowerCase(edtSearch.Text, loUserLocale),
        LowerCase(lvCommands.Items[current].SubItems[2], loUserLocale)) > 0)
      then
        Inc(current);
      ...
      begin
        if FCommands[i].HasData(edtSearch.Text) or
          (Pos(LowerCase(edtSearch.Text, loUserLocale),
          LowerCase(lvCommands.Items[i].SubItems[2], loUserLocale)) > 0) then
        begin
          lvCommands.ClearSelection;
          lvCommands.Items[i].Selected := True;
          lvCommands.Items[i].MakeVisible(True);
      ...
      if Application.MessageBox('Начать с начала?', 'Поиск завершен',
    ...
    procedure TfmMain.lvCommandsAdvancedCustomDrawItem(Sender: TCustomListView;
      Item: TListItem; State: TCustomDrawState; Stage: TCustomDrawStage;
      var DefaultDraw: Boolean);
    begin
    ...
    procedure TfmMain.lvCommandsAdvancedCustomDrawSubItem
    ...
    begin
      ...
      memInfo.Lines.Add(Command.Attributes + ' ' + protocolstr + ' (Line ' +
        Command.LineNumber.ToString + ')');
      memInfo.Lines.Add(Command.CommandName);
      memInfo.Lines.Add('Передано     : ' + Command.TxData);
    ...
    begin
      ...
      try
        for Command in FCommands do
        begin
          S := Format('[%s] [%s] [%s]'#13#10,
            [Command.TimeStamp, Command.ThreadID, Command.CommandName]);
          Bytes := TEncoding.ANSI.GetBytes(S);
          F.Write(Bytes, Length(Bytes));
          S := Format('[%s] [%s] -> %s'#13#10,
            [Command.TimeStamp, Command.ThreadID, Command.TxData]);
          Bytes := TEncoding.ANSI.GetBytes(S);
          F.Write(Bytes, Length(Bytes));
          if Command.AnswerData <> '' then
          begin
            S := Format('[%s] [%s] <- %s'#13#10,
              [Command.TimeStamp, Command.ThreadID, Command.AnswerData]);
            Bytes := TEncoding.ANSI.GetBytes(S);
            F.Write(Bytes, Length(Bytes));
          end;
        end;
    ...
    begin
      ...
      for i := 0 to lvCommands.Items.Count - 1 do
      begin
        memInfo.Lines.Add('lv ' + i.ToString + ' ' + lvCommands.Items
          [i].Caption);
    ...
    begin
      ...
      if CycleNumber >= 0 then
        edtStatus.Text := Format('Цикл %d (%d/%d) %s: Ошибка: %s',
          [CycleNumber, Index + 1, FCommands.Count,
          FCommands[Index].CommandName, ErrorText])
      else
        edtStatus.Text := Format('(%d/%d) %s: Ошибка: %s',
          [Index + 1, FCommands.Count, FCommands[Index].CommandName,
          ErrorText]);
    end;
    ...
    begin
      ...
      if Application.MessageBox(PWideChar('Ошибка: ' + AMsg + #13#10 +
        'Продолжить?'), 'Ошибка', MB_YESNO + MB_ICONSTOP) = IDYES then
      begin
        FPlayer.PlayMode := pmContinue;
      end
      else
    ...
    begin
      ...
      if ACycleNumber >= 0 then
        edtStatus.Text := Format('Цикл %d (%d/%d) %s',
          [ACycleNumber, Index + 1, FCommands.Count,
          FCommands[Index].CommandName])
      else
        edtStatus.Text := Format('(%d/%d) %s', [Index + 1, FCommands.Count,
          FCommands[Index].CommandName]);
    end;
